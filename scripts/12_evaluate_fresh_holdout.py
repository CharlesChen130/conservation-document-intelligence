"""Execute the newly frozen untouched holdout exactly once."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Execute the frozen post-generalization holdout once."
    )
    parser.add_argument(
        "--with-openai",
        action="store_true",
    ...
    parser.add_argument(
        "--spec",
        type=Path,
        default=PROJECT_ROOT / "data" / "holdout_v2_spec.yaml",
        help="Newly frozen holdout specification to execute.",
    )
    parser.add_argument(
        "--artifact-prefix",
        default="holdout_v2",
        help="Immutable first-run output filename prefix under outputs/.",
    )
...
def main() -> None:
    ...
    report_path = OUTPUTS_DIR / f"{args.artifact_prefix}_answers.md"
    ...
    if report_path.exists():
        raise FileExistsError(
            f"Fresh holdout report already exists and is immutable: {report_path}"
        )
    ...
    print(f"Executed fresh frozen holdout: {dict(sorted(counts.items()))}")
    print(
        "Usage: "

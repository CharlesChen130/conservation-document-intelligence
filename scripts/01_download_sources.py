...
from src.conservation_intelligence.acquisition import acquire_catalog
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Acquire the public conservation corpus.")
    parser.add_argument(
        "--doc-id",
        action="append",
        dest="doc_ids",
        help="Acquire only this ID; may be repeated.",
    )
    parser.add_argument("--limit", type=int, help="Acquire only the first N selected records.")
    parser.add_argument("--timeout", type=float, default=45.0, help="Per-request timeout in seconds.")
    parser.add_argument("--force", action="store_true", help="Redownload files whose checksums are valid.")
...
def main() -> None:
    ...
    for result in results:
        print(f"{result.doc_id}: {result.status} {result.local_file or result.note}")
